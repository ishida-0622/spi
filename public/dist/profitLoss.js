...
class profitLoss {
    easy(rep) {
        ...
        $("#ans").html(optHtml);
        const res = { profitLoss: { ans: selling, cost: cost, profit: profit, regular: regular, discount: discount, selling: selling, html: html } };
        return res;
    }
    normal(rep) {
        const cost = 100;
        ...
        const optHtml = optHtmlCreate(opt);
        $("#ans").html(optHtml);
        const res = { profitLoss: { ans: ans, cost: cost, profit: profit, regular: regular, discount: discount, selling: selling, html: html } };
        return res;
    }
    hard(rep) {
        ...
        const optHtml = optHtmlCreate(opt);
        $("#ans").html(optHtml);
        const res = { profitLoss: { ans: cost, cost: cost, profit: profit, regular: regular, discount: discount, selling: selling, html: html } };
        return res;
    }
}
const profitLossResult = (userAns, dic, diff) => {
    let html;
    if (diff === diffList.e) {
        html = `
