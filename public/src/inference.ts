class inference implements q {
    easy(rep: number): dict {
        ...
        const B =
            arr[0] === "B" ? 1 : arr[1] === "B" ? 2 : arr[2] === "B" ? 3 : 4;
        const C =
            arr[0] === "C" ? 1 : arr[1] === "C" ? 2 : arr[2] === "C" ? 3 : 4;
        const D =
...
const inferenceResult = (
    ...
    if (diff === diffList.e) {
        ...
        ${notA[0]}は${notA[2]}よりも上の順位なので、${notA[0]} > ${
            notA[2]
        }となるが、正確な順位は分からない。<br>
        ${notA[1]}は${notA[0]}と${notA[2]}の間なので、${notA[0]} > ${
            notA[1]
        } > ${notA[2]}となる。<br>
        Aの順位は確定しているため、残りの3枠に当てはめると「${
            dic.inference!.arr
        }」となる。
        </p>
